...
vhost localhost {
    location "/" {
        proxy { upstream "unix:$sock"; }
    }
}
EOF
    start_server "$TMPDIR/proxy_unix.kdl" 8090 \
        || { fail "proxy_unix/server_start" "aloha failed"; return; }
...
vhost localhost {
    location "/" {
        proxy { upstream "http://127.0.0.1:9007"; }
    }
}
EOF
    start_server "$TMPDIR/proxy_xff.kdl" 8105 \
        || { fail "proxy_xff/server_start" "aloha failed"; return; }
...
vhost localhost {
    location "/api/" {
        proxy { upstream "http://127.0.0.1:9008"; strip-prefix #true; }
    }
}
EOF
    start_server "$TMPDIR/proxy_strip.kdl" 8106 \
        || { fail "proxy_strip/server_start" "aloha failed"; return; }
