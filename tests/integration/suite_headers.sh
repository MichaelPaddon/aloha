...
EOF
    start_server "$TMPDIR/resphdrs.kdl" 8096 \
        || { fail "resp_headers/server_start" "aloha failed"; return; }
...
vhost localhost {
    location "/" {
        request-headers {
            set "X-Injected" "hello-from-aloha"
        }
        proxy { upstream "http://127.0.0.1:9004"; }
    }
}
EOF
    start_server "$TMPDIR/reqhdrs.kdl" 8097 \
        || { fail "req_headers/server_start" "aloha failed"; return; }

    assert_body "req_headers/injected" "hello-from-aloha" \
        "http://127.0.0.1:8097/"
