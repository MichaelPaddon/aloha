...
vhost "site-a.local" {
    location "/" {
        static { root "/tmp/www-a"; index-file index.html; }
    }
}
vhost "site-b.local" {
    location "/" {
        static { root "/tmp/www-b"; index-file index.html; }
    }
}
EOF
    start_server "$TMPDIR/multi.kdl" 8093 \
        || { fail "multi_vhost/server_start" "aloha failed"; return; }
...
EOF
    start_server "$TMPDIR/alias.kdl" 8094 \
        || { fail "aliases/server_start" "aloha failed"; return; }
...
EOF
    start_server "$TMPDIR/regex.kdl" 8095 \
        || { fail "regex_vhost/server_start" "aloha failed"; return; }
