...
suite_proxy_skip_verify() {
    ...
    # Upstream: a second aloha instance with a self-signed cert.
    cat >"$TMPDIR/upstream_tls.kdl" <<'EOF'
listener {
    bind "127.0.0.1:18450"
    tls-self-signed
}
vhost localhost {
    location "/" {
...
EOF
    "$ALOHA" --config "$TMPDIR/upstream_tls.kdl" \
...
EOF
    ...
    assert_body   "proxy_skip_verify/body" "Hello aloha" \
        "http://127.0.0.1:8091/"
...
EOF
    ...
    # before timing out 60 times.  Sleep briefly and verify aloha
    # bound the listener via /dev/tcp.
    "$ALOHA" --config "$TMPDIR/proxy_ctimeout.kdl" \
