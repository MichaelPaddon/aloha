...
# Two aloha configs are tested: one pointing at /allow (returns 200 →
...
server {
    auth subrequest "http://127.0.0.1:9009/allow"
}
listener {
    bind "127.0.0.1:8103"
}
...
EOF
    start_server "$TMPDIR/subreq_allow.kdl" 8103 \
        || { fail "subreq/allow_server_start" "aloha failed"; return; }
...
server {
    auth subrequest "http://127.0.0.1:9009/deny"
}
listener {
    bind "127.0.0.1:8103"
}
...
EOF
    start_server "$TMPDIR/subreq_deny.kdl" 8103 \
        || { fail "subreq/deny_server_start" "aloha failed"; return; }
