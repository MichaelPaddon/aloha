...
EOF
    ...
    "$ALOHA" --config "$TMPDIR/h3.kdl" >"$TMPDIR/aloha.out" 2>&1 &
    ALOHA_PID=$!
    sleep 0.5
    if ! kill -0 "$ALOHA_PID" 2>/dev/null; then
        fail "h3/server_start" "aloha exited during startup"
    ...
    assert_h3_body   "h3/body" "Hello aloha" "https://127.0.0.1:18443/"
...
EOF
    "$ALOHA" --config "$TMPDIR/h3_mid.kdl" >"$TMPDIR/aloha.out" 2>&1 &
    ALOHA_PID=$!
    sleep 0.5
    if ! kill -0 "$ALOHA_PID" 2>/dev/null; then
        fail "h3_mid/server_start" "aloha exited during startup"
        cat "$TMPDIR/aloha.out" >&2
        ALOHA_PID=""
        return
    fi
    ...
    assert_h3_header "h3_mid/open_header" \
        "x-aloha-middleware" "h3-saw-this" \
        "https://127.0.0.1:18445/open"
...
listener { bind "127.0.0.1:18444";     tls-self-signed }
listener { bind "udp:127.0.0.1:18444"; tls-self-signed }
vhost localhost {
    location "/" {
        static { root "/tmp/www"; index-file index.html; }
    }
...
EOF
    "$ALOHA" --config "$TMPDIR/h3_altsvc.kdl" \
