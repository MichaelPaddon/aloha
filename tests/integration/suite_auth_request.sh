#!/bin/bash
# Suite: auth-request handler (aloha as the auth decision endpoint).
#
...
EOF
    start_server "$TMPDIR/auth_request.kdl" 8104 \
        || { fail "auth_request/server_start" "aloha failed"; return; }
    ...
    assert_header "auth_request/user_header"  "X-Auth-User" "alohatest" \
        "http://127.0.0.1:8104/" -u "alohatest:alohapass"
