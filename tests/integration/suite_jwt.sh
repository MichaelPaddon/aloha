...
EOF
    start_server "$TMPDIR/jwt.kdl" 8102 \
        || { fail "jwt/server_start" "aloha failed"; return; }
    ...
    local auth_code
    auth_code=$(curl -s -u "alohatest:alohapass" \
    ...
    if grep -q "aloha_session" "$cookie_jar" 2>/dev/null; then
        pass "jwt/cookie_issued"
    else
        fail "jwt/cookie_issued" "Set-Cookie for aloha_session not found"
    fi
    ...
    assert_status "jwt/bad_cookie_401" 401 "http://127.0.0.1:8102/" \
        -H "Cookie: aloha_session=garbage.token.value"
