...
#
# Spawns a second aloha instance as the upstream so the frontend
...
# Start an upstream aloha listening on the given TCP+UDP port pair.
# Same-port listeners trigger aloha's auto-Alt-Svc behaviour: TCP
# responses advertise the matching h3 endpoint, which is exactly
# what the auto-upgrade path in the proxy looks for.
_start_h3_upstream() {
    local port="$1" cfg="$2"
    cat >"$cfg" <<EOF
listener { bind "127.0.0.1:${port}";       tls-self-signed }
listener { bind "udp:127.0.0.1:${port}";   tls-self-signed }
vhost localhost {
    location "/" {
...
EOF
    "$ALOHA" --config "$cfg" >"$TMPDIR/upstream.out" 2>&1 &
    UPSTREAM_PID=$!
...
EOF
    ...
    assert_body   "proxy_h3/body" "Hello aloha" "http://127.0.0.1:8087/"
...
listener { bind "127.0.0.1:18447";     tls-self-signed }
listener { bind "udp:127.0.0.1:18447"; tls-self-signed }
vhost localhost {
    location "/" {
        static { root "/tmp/www"; index-file index.html; }
        response-headers {
...
EOF
    "$ALOHA" --config "$TMPDIR/upstream_ttl.kdl" \
...
EOF
    "$ALOHA" --config "$TMPDIR/proxy_ttl.kdl" \
        >"$TMPDIR/aloha.out" 2>&1 &
    ALOHA_PID=$!
    tries=0
...
EOF
    ...
    "$ALOHA" --config "$TMPDIR/proxy_auto.kdl" \
        >"$TMPDIR/aloha.out" 2>&1 &
    ALOHA_PID=$!
    local tries=0
