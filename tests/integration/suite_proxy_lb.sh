...
EOF
    start_server "$TMPDIR/lb_rr.kdl" 8201 \
        || { fail "lb_rr/server_start" "aloha failed"; return; }
...
EOF
    start_server "$TMPDIR/lb_hh.kdl" 8211 \
        || { fail "lb_hh/server_start" "aloha failed"; return; }
...
vhost localhost {
    location "/" {
        proxy {
            ...
            lb-policy "round-robin"
            retry { max 1; on-status 503 }
        }
    }
}
EOF
    start_server "$TMPDIR/lb_retry.kdl" 8221 \
        || { fail "lb_retry/server_start" "aloha failed"; return; }
...
EOF
    start_server "$TMPDIR/lb_hc.kdl" 8231 \
        || { fail "lb_hc/server_start" "aloha failed"; return; }
