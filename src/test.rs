...
//
// Provides TestServer (an aloha HTTP server with auto-shutdown) and
...
/// In-process aloha HTTP server for component tests.
...
impl TestServer {
    ...
    pub async fn start(template: &str) -> Self {
        ...
        // Choose socket type based on the bind address in the config.
        let addr = if cfg.bind.starts_with("unix:") {
            #[cfg(unix)]
            {
                drop(tcp_listener); // TCP port not needed
                let path = cfg.bind.strip_prefix("unix:").unwrap();
                let unix_listener =
