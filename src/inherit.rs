...
impl InheritedSockets {
    ...
    pub fn warn_unclaimed(&self) {
        ...
        for (addr, fd) in &self.udp {
            tracing::warn!(
                fd, %addr,
                "inherited UDP socket not claimed by any QUIC listener \
                 (bind address mismatch?)"
            );
        }
    }
...
/// SOCK_DGRAM socket with an INET local address is what aloha needs to
...
fn classify_fd(
    ...
    // from a parent aloha across a seamless upgrade).
